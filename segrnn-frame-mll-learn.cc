    ...
    : args(args)
{
    ...
    layer = std::stoi(line);
    nn_param = lstm_frame::make_tensor_tree(layer);
    ...
    layer = std::stoi(line);
    nn_opt_data = lstm_frame::make_tensor_tree(layer);
...
void learning_env::run()
{
    ...
    while (nsample < frame_batch.pos.size()) {
        ...
        std::vector<speech::segment> segs = speech::load_segment_batch(label_batch.at(nsample));

        std::vector<int> label_seq;
        for (auto& s: segs) {
            label_seq.push_back(l_args.label_id.at(s.label));
        }
        ...
        std::shared_ptr<lstm::transcriber> trans
            = lstm_frame::make_pyramid_transcriber(layer, dropout, &l_args.gen);
        ...
        autodiff::eval(logprob_order, autodiff::eval_funcs);
        ...
        if (ell > 0) {
            ...
            std::vector<std::shared_ptr<tensor_tree::vertex>> vars
                = tensor_tree::leaves_pre_order(nn_param);
    ...
    std::ofstream nn_param_ofs { args.at("output-nn-param") };
    nn_param_ofs << layer << std::endl;
    tensor_tree::save_tensor(nn_param, nn_param_ofs);
    nn_param_ofs.close();

    std::ofstream nn_opt_data_ofs { args.at("output-nn-opt-data") };
